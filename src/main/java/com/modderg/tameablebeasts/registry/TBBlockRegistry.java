...
import static com.modderg.tameablebeasts.registry.TBItemRegistry.ITEMS;

public class TBBlockRegistry {
    public static final DeferredRegister<Block> BLOCKS = DeferredRegister.create(ForgeRegistries.BLOCKS, TameableBeasts.MOD_ID);
    ...
    public static final RegistryObject<Block> ASPHALTED_DIRT = register("asphalted_dirt", () -> new DirtPathBlock(BlockBehaviour.Properties
            .of().mapColor(MapColor.DIRT).strength(0.65F).sound(SoundType.GRASS).isViewBlocking(TBBlockRegistry::always).isSuffocating(TBBlockRegistry::always)),
            object -> () -> new BlockItem(object.get(), new Item.Properties()));
